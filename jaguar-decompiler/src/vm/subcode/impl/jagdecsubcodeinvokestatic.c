/*
   File:    jagdecsubcodeinvokestatic.c
...
#include "jagdecsubcodeinvokestatic.h"
...
#define CAT_LOG_CLAZZ "JagDecSubCodeInvokeStatic"
#include <logging/catlog.h>

struct _JagDecSubCodeInvokeStaticPrivate {
	JagAstDeclarationType *class_decl_type;
...
G_DEFINE_TYPE_WITH_CODE(JagDecSubCodeInvokeStatic, jag_dec_sub_code_invoke_static, JAG_DEC_TYPE_ABSTRACT_SUB_CODE, {
...
static void jag_dec_sub_code_invoke_static_class_init(JagDecSubCodeInvokeStaticClass *clazz) {
	parent_class = g_type_class_peek_parent(clazz);
	g_type_class_add_private(clazz, sizeof(JagDecSubCodeInvokeStaticPrivate));
...
static void jag_dec_sub_code_invoke_static_init(JagDecSubCodeInvokeStatic *instance) {
	JagDecSubCodeInvokeStaticPrivate *priv = G_TYPE_INSTANCE_GET_PRIVATE(instance, JAG_DEC_TYPE_SUB_CODE_INVOKE_STATIC, JagDecSubCodeInvokeStaticPrivate);
	instance->priv = priv;
}

static void l_dispose(GObject *object) {
	cat_log_detail("dispose:%p", object);
	JagDecSubCodeInvokeStatic *instance = JAG_DEC_SUB_CODE_INVOKE_STATIC(object);
	JagDecSubCodeInvokeStaticPrivate *priv = instance->priv;
	cat_unref_ptr(priv->class_decl_type);
...
JagDecSubCodeInvokeStatic *jag_dec_sub_code_invoke_static_new(int mnemonic_index, JagAstDeclarationType *class_decl_type, JagBytMethodHeader *method_header, int argument_count) {
	JagDecSubCodeInvokeStatic *result = g_object_new(JAG_DEC_TYPE_SUB_CODE_INVOKE_STATIC, NULL);
	cat_ref_anounce(result);
	JagDecSubCodeInvokeStaticPrivate *priv = result->priv;
	jag_dec_abstract_sub_code_construct((JagDecAbstractSubCode *) result, mnemonic_index);
	priv->class_decl_type = cat_ref_ptr(class_decl_type);
...
static void l_sub_code_connect_stage_two(JagDecISubCode *self, JagDecModelStageTwoRuntime *modelRuntime) {
	JagDecSubCodeInvokeStaticPrivate *priv = JAG_DEC_SUB_CODE_INVOKE_STATIC_GET_PRIVATE(self);
	...
	JagAstIExpression *instance_expression = jag_dec_iconsumer_get_expression_ref(jag_dec_isub_code_get_input_consumer_at(self, priv->argument_count), priv->class_decl_type);
