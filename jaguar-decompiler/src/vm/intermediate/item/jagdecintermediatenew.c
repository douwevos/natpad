...
#include "jagdecintermediatenew.h"
#include "../../subcode/jagdecivalueproducer.h"
#include "../../subcode/jagdecdefaultvalueproducer.h"
#include "../../subcode/jagdecisubcode.h"
#include "../../subcode/impl/jagdecsubcodenew.h"


#include <logging/catlogdefs.h>
#define CAT_LOG_LEVEL CAT_LOG_WARN
#define CAT_LOG_CLAZZ "JagDecIntermediateNew"
#include <logging/catlog.h>

struct _JagDecIntermediateNewPrivate {
	JagAstDeclarationType *declaration_type;
};

static void l_intermediate_node_iface_init(JagDecIIntermediateNodeInterface *iface);

G_DEFINE_TYPE_WITH_CODE(JagDecIntermediateNew, jag_dec_intermediate_new, JAG_DEC_TYPE_ABSTRACT_INTERMEDIATE_ITEM, {
...
static void jag_dec_intermediate_new_class_init(JagDecIntermediateNewClass *clazz) {
	parent_class = g_type_class_peek_parent(clazz);
	g_type_class_add_private(clazz, sizeof(JagDecIntermediateNewPrivate));
...
static void jag_dec_intermediate_new_init(JagDecIntermediateNew *instance) {
	JagDecIntermediateNewPrivate *priv = G_TYPE_INSTANCE_GET_PRIVATE(instance, JAG_DEC_TYPE_INTERMEDIATE_NEW, JagDecIntermediateNewPrivate);
	instance->priv = priv;
}

static void l_dispose(GObject *object) {
	cat_log_detail("dispose:%p", object);
	JagDecIntermediateNew *instance = JAG_DEC_INTERMEDIATE_NEW(object);
	JagDecIntermediateNewPrivate *priv = instance->priv;
...
JagDecIntermediateNew *jag_dec_intermediate_new_new(int mnemonic_index, JagAstDeclarationType *declaration_type) {
	JagDecIntermediateNew *result = g_object_new(JAG_DEC_TYPE_INTERMEDIATE_NEW, NULL);
	cat_ref_anounce(result);
	JagDecIntermediateNewPrivate *priv = result->priv;
...
static void l_intermediate_node_connect_stage_one(JagDecIIntermediateNode *self, JagDecIntermediateRunContext *group_connect_runtime) {
	JagDecIntermediateNewPrivate *priv = JAG_DEC_INTERMEDIATE_NEW_GET_PRIVATE(self);
	int mnemonic_index = jag_dec_abstract_intermediate_item_get_mnemonic_index((JagDecAbstractIntermediateItem *) self);
	JagDecSubCodeNew *sub_code_new = jag_dec_sub_code_new_new(mnemonic_index, priv->declaration_type);
	...
	JagDecValueTypeDefinition *definition = jag_dec_value_type_definition_new(priv->declaration_type, JAG_DEC_VALUE_TYPE_DEFINITION_KIND_NEW);
	jag_dec_value_type_definition_list_add_definition(type_set, definition);
	cat_unref(definition);

	jag_dec_abstract_sub_code_set_value_producer((JagDecAbstractSubCode *) sub_code_new, value_producer);
	jag_dec_intermediate_run_context_push_value_provider(group_connect_runtime, value_producer);
	jag_dec_intermediate_run_context_add_sub_code(group_connect_runtime, (JagDecISubCode *) sub_code_new);
	cat_unref_ptr(sub_code_new);
...
static gboolean l_intermediate_node_is_same(JagDecIIntermediateNode *self, JagDecIIntermediateNode *other_node) {
	JagDecIntermediateNewPrivate *priv = JAG_DEC_INTERMEDIATE_NEW_GET_PRIVATE(self);
	if (JAG_DEC_IS_INTERMEDIATE_NEW(other_node)) {
		JagDecIntermediateNewPrivate *o_priv = JAG_DEC_INTERMEDIATE_NEW_GET_PRIVATE(other_node);
		return jag_ast_declaration_type_equal(priv->declaration_type, o_priv->declaration_type);
	}
	return FALSE;
}
...
static JagDecIIntermediateNode *l_intermediate_node_make_copy(JagDecIIntermediateNode *self, gboolean deep) {
	JagDecIntermediateNewPrivate *priv = JAG_DEC_INTERMEDIATE_NEW_GET_PRIVATE(self);
	int mnemonic_index = jag_dec_abstract_intermediate_item_get_mnemonic_index((JagDecAbstractIntermediateItem *) self);
	return (JagDecIIntermediateNode *) jag_dec_intermediate_new_new(mnemonic_index, priv->declaration_type);
}
