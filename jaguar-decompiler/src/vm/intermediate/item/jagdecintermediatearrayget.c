/*
   File:    jagdecintermediatearrayget.c
...
#include "jagdecintermediatearrayget.h"
#include "jagdecarraygetinstanceconsumer.h"
#include "../../subcode/jagdecconsumer.h"
#include "../../subcode/impl/jagdecsubcodearrayget.h"
#include "../../subcode/jagdecdefaultvalueproducer.h"

#include <logging/catlogdefs.h>
#define CAT_LOG_LEVEL CAT_LOG_WARN
#define CAT_LOG_CLAZZ "JagDecIntermediateArrayGet"
#include <logging/catlog.h>

struct _JagDecIntermediateArrayGetPrivate {
	JagBytType load_type;
};

static void l_intermediate_node_iface_init(JagDecIIntermediateNodeInterface *iface);
static void l_stringable_iface_init(CatIStringableInterface *iface);

G_DEFINE_TYPE_WITH_CODE(JagDecIntermediateArrayGet, jag_dec_intermediate_array_get, JAG_DEC_TYPE_ABSTRACT_INTERMEDIATE_ITEM, {
...
static void jag_dec_intermediate_array_get_class_init(JagDecIntermediateArrayGetClass *clazz) {
	parent_class = g_type_class_peek_parent(clazz);
	g_type_class_add_private(clazz, sizeof(JagDecIntermediateArrayGetPrivate));
...
static void jag_dec_intermediate_array_get_init(JagDecIntermediateArrayGet *instance) {
	JagDecIntermediateArrayGetPrivate *priv = G_TYPE_INSTANCE_GET_PRIVATE(instance, JAG_DEC_TYPE_INTERMEDIATE_ARRAY_GET, JagDecIntermediateArrayGetPrivate);
	instance->priv = priv;
}

static void l_dispose(GObject *object) {
	cat_log_detail("dispose:%p", object);
//	JagDecIntermediateArrayGet *instance = JAG_DEC_INTERMEDIATE_ARRAY_GET(object);
//	JagDecIntermediateArrayGetPrivate *priv = instance->priv;
...
JagDecIntermediateArrayGet *jag_dec_intermediate_array_get_new(int mnemonic_index, JagBytType load_type) {
	JagDecIntermediateArrayGet *result = g_object_new(JAG_DEC_TYPE_INTERMEDIATE_ARRAY_GET, NULL);
	cat_ref_anounce(result);
	JagDecIntermediateArrayGetPrivate *priv = result->priv;
	jag_dec_abstract_intermediate_item_construct((JagDecAbstractIntermediateItem *) result, mnemonic_index);
	priv->load_type = load_type;
	return result;
}
...
static void l_intermediate_node_connect_stage_one(JagDecIIntermediateNode *self, JagDecIntermediateRunContext *group_connect_runtime) {
	...
	JagDecSubCodeArrayGet *sub_code = jag_dec_sub_code_array_get_new(mnemonic_index);
	...
	JagDecDefaultValueProducer *producer = jag_dec_default_value_producer_new(self);
	JagDecConsumer *instance_consumer = (JagDecConsumer *) jag_dec_array_get_instance_consumer_new(self, (JagDecIValueProducer *) producer);

	jag_dec_abstract_sub_code_add_consumer((JagDecAbstractSubCode *) sub_code, (JagDecIConsumer *) instance_consumer);
	jag_dec_abstract_sub_code_set_value_producer((JagDecAbstractSubCode *) sub_code, (JagDecIValueProducer *) producer);

	jag_dec_intermediate_run_context_add_and_run_sub_code(group_connect_runtime, (JagDecISubCode *) sub_code);

	JagDecValueTypeDefinitionList *type_set = jag_dec_ivalue_producer_get_type_set((JagDecIValueProducer *) producer);
	jag_dec_intermediate_run_context_add_value_type_definition_list(group_connect_runtime, type_set);
...
static gboolean l_intermediate_node_is_same(JagDecIIntermediateNode *self, JagDecIIntermediateNode *other_node) {
	JagDecIntermediateArrayGetPrivate *priv = JAG_DEC_INTERMEDIATE_ARRAY_GET_GET_PRIVATE(self);
	if (JAG_DEC_IS_INTERMEDIATE_ARRAY_GET(other_node)) {
		JagDecIntermediateArrayGetPrivate *o_priv = JAG_DEC_INTERMEDIATE_ARRAY_GET_GET_PRIVATE(other_node);
		return priv->load_type==o_priv->load_type;
	}
	return FALSE;
}
...
static JagDecIIntermediateNode *l_intermediate_node_make_copy(JagDecIIntermediateNode *self, gboolean deep) {
	JagDecIntermediateArrayGetPrivate *priv = JAG_DEC_INTERMEDIATE_ARRAY_GET_GET_PRIVATE(self);
	int mnemonic_index = jag_dec_abstract_intermediate_item_get_mnemonic_index((JagDecAbstractIntermediateItem *) self);
	return (JagDecIIntermediateNode *) jag_dec_intermediate_array_get_new(mnemonic_index, priv->load_type);
}
...
static void l_stringable_print(CatIStringable *self, struct _CatStringWo *buf) {
	...
	cat_string_wo_append_chars(buf, "[load-type=");
	cat_string_wo_append_chars(buf, jag_byt_type_as_text(priv->load_type));
