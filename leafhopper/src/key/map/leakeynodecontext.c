...
G_DEFINE_TYPE_WITH_PRIVATE(LeaKeyNodeContext, lea_key_node_context, LEA_TYPE_KEY_NODE)
...
static void l_dispose(GObject *object) {
	...
	LeaKeyNodeContextPrivate *priv = lea_key_node_context_get_instance_private(instance);
...
LeaKeyNodeContext *lea_key_node_context_new(LeaKeyContext *key_context) {
	...
	LeaKeyNodeContextPrivate *priv = lea_key_node_context_get_instance_private(result);
...
static int l_node_hash(LeaKeyNode *self) {
	LeaKeyNodeContext *instance = LEA_KEY_NODE_CONTEXT(self);
	LeaKeyNodeContextPrivate *priv = lea_key_node_context_get_instance_private(instance);
...
static gboolean l_node_equal(LeaKeyNode *node_a, LeaKeyNode *node_b) {
	...
	LeaKeyNodeContextPrivate *priv_a = lea_key_node_context_get_instance_private(LEA_KEY_NODE_CONTEXT(node_a));
	LeaKeyNodeContextPrivate *priv_b = lea_key_node_context_get_instance_private(LEA_KEY_NODE_CONTEXT(node_b));
