...
SmplVirtualParseStack *smpl_virtual_parse_stack_new(CatArrayWo *e_real) {
...
void smpl_virtual_parser_stack_get_from_real(SmplVirtualParseStack *virtual_stack) {
	...
	stack_sym = (SmplToken *) cat_array_wo_get(virtual_stack->e_real_stack, index);
...
//		throw new Exception("Internal parser error: top() called on empty virtual stack");
	...
	SmplToken *top_token = (SmplToken *) cat_array_wo_get(virtual_stack->e_vstack, cat_array_wo_size(virtual_stack->e_vstack)-1);
