#include "jagparserbase.h"

#include <logging/catlogdefs.h>
#define CAT_LOG_LEVEL CAT_LOG_WARN
#define CAT_LOG_CLAZZ "JagParserBase"
#include <logging/catlog.h>

G_DEFINE_TYPE(JagParserBase, jag_parser_base, G_TYPE_OBJECT)

static void _dispose(GObject *object);

static void jag_parser_base_class_init(JagParserBaseClass *clazz) {
	GObjectClass *object_class = G_OBJECT_CLASS(clazz);
	object_class->dispose = _dispose;
}

static void jag_parser_base_init(JagParserBase *obj) {
}

static void _dispose(GObject *object) {
//	JagParserBase *instance = JAG_PARSER_BASE(object);
	// TODO
}


JagParserBase *jag_parser_base_new() {
	JagParserBase *result = g_object_new(JAG_TYPE_PARSER_BASE, NULL);
...
JagToken *jag_parser_base_parse(JagParserBase *parser_base, JagParserContext *parserContext) {
	...
	JagToken *lhs_sym = NULL;

	/* information about production being reduced with */
	short handle_size, lhs_sym_num;


	const JagParserBaseClass *base_class = JAG_PARSER_BASE_GET_CLASS(parser_base);
	const JagParserContextClass *parser_context_class = JAG_PARSER_CONTEXT_GET_CLASS(parserContext);
	...
	JagIScannerInterface *scanner_iface = JAG_ISCANNER_GET_INTERFACE(parserContext->scanner);
	
	JagToken *start_token = scanner_iface->createToken(parserContext->scanner, 0, base_class->start_state(parser_base), FALSE, FALSE, FALSE, 0, 0, 0, 0, NULL);
	parser_context_class->push(parserContext, start_token);	
	cat_unref_ptr(start_token);
//	parser_context_class->push(parserContext, jag_token_new_symbol_state(0, base_class->start_state(parser_base)));
	parserContext->tos = 0;
...
//			throw new Error("Symbol recycling detected (fix your scanner).");
		...
		jag_parser_context_debug(parserContext);
		jag_token_dump(parser_context_class->peek(parserContext));

		if (parserContext->current_token!=NULL) {
			jag_token_dump(parserContext->current_token);
		}
		...
		act = jag_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, parserContext->current_token->sym);
		...
		if (act > 0) {
			...
			lhs_sym_num = jag_2d_array_get(parser_base->production_tab, (-act) - 1, 0);
			handle_size = jag_2d_array_get(parser_base->production_tab, (-act) - 1, 1);

			act = jag_parser_base_get_reduce(parser_base, parser_context_class->peek_at(parserContext, handle_size)->parse_state, lhs_sym_num);
...
//			act = jag_parser_base_get_reduce(parser_base, parser_context_class->peek(parserContext)->parse_state, lhs_sym_num);
//
//			/* shift to that state */
//			lhs_sym->parse_state = act;
//			lhs_sym->used_by_parser = TRUE;
...
//			jag_parser_report_syntax_error(parserContext->current_token);

			/* try to error recover */
			if (!jag_parser_base_error_recovery(parser_base, parserContext, FALSE)) {
				/* if that fails give up with a fatal syntax error */
//				jag_parser_unrecovered_syntax_error(parser_base, parserContext, parserContext->current_token);

				/* just in case that wasn't fatal enough, end parse */
				jag_parser_context_done_parsing(parserContext);
				lhs_sym = NULL;
			} else {
...
short jag_parser_base_get_action(JagParserBase *parser_base, int state, int sym) {
	...
	int row_length = 0;
	short *row = jag_2d_array_get_row(parser_base->action_tab, state, &row_length);
...
gboolean jag_parser_base_error_recovery(JagParserBase *parser_base, JagParserContext *parserContext, gboolean debug) {
	JagParserBaseClass *base_class = JAG_PARSER_BASE_GET_CLASS(parser_base);
	 ...
	 */
	if (!jag_parser_base_find_recovery_config(parser_base, parserContext, debug)) {
//		if (debug) cat_log_debug("# Error recovery fails");
		return FALSE;
	}

	/* read ahead to create lookahead we can parse multiple times */
	jag_parser_base_read_lookahead(parser_base, parserContext);
	...
	while(TRUE) {
		...
		if (jag_parser_base_try_parse_ahead(parser_base, parserContext, debug)) {
			break;
		}

		/* if we are now at EOF, we have failed */
		JagToken *bottomToken = (JagToken *) cat_array_wo_get(parserContext->e_lookahead, 0);
		if (bottomToken==NULL) {
...
//		cat_log_debug("# Consuming Symbol #" + parserContext->e_lookahead[0].sym);
		...
		jag_parser_base_restart_lookahead(parser_base, parserContext);
	}
	...
	jag_parser_base_parse_lookahead(parser_base, parserContext, debug);
...
short jag_parser_base_get_reduce(JagParserBase *parser_base, int state, int sym) {
	...
	short *row = jag_2d_array_get_row(parser_base->reduce_tab, state, &row_length);
...
gboolean jag_parser_base_shift_under_error(JagParserBase *parser_base, JagParserContext *parserContext) {
	...
	int errorSym = JAG_PARSER_BASE_GET_CLASS(parser_base)->error_symbol(parser_base);
	JagParserContextClass *parser_context_class = JAG_PARSER_CONTEXT_GET_CLASS(parserContext);
	return jag_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, errorSym) > 0;
}
...
gboolean jag_parser_base_find_recovery_config(JagParserBase *parser_base, JagParserContext *parserContext, gboolean debug) {
	...
	while (!jag_parser_base_shift_under_error(parser_base, parserContext)) {
		/* pop the stack */
//		if (debug) cat_log_debug("# Pop stack by one, state was # " + parser_context_class->peek(parserContext).parse_state);
	...
	act = jag_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, base_class->error_symbol(parser_base));
//	if (debug) {
...
//	error_token = jag_token_new_full(base_class->error_symbol(parser_base), FALSE, left_pos, left_row, right_pos, right_row, NULL);
...
//		parserContext.lookahead = new JagToken[jag_parser_error_sync_size()];
	} else {
...
void jag_parser_base_restart_lookahead(JagParserBase *parser_base, JagParserContext *parserContext) {
	...
	int eofSymbol = JAG_PARSER_BASE_GET_CLASS(parser_base)->eof_symbol(parser_base);
	parser_context_class->scanNext(parserContext, eofSymbol);
...
gboolean jag_parser_base_try_parse_ahead(JagParserBase *parser_base, JagParserContext *parserContext, gboolean debug) {
	...
	JagVirtualParseStack *vstack = jag_virtual_parse_stack_new(parserContext->e_stack);
	...
	while(TRUE) {
		JagToken *error_token = jag_parser_context_current_error_token(parserContext);
		...
		int act = jag_parser_base_get_action(parser_base, jag_virtual_parser_stack_top(vstack), error_token->sym);
		...
		if (act > 0) {
			...
			JagToken *token2push = scanner_iface->createToken(parserContext->scanner, 0, act-1, FALSE, FALSE, FALSE, 0, 0, 0, 0, NULL);
			jag_virtual_parser_stack_push(vstack, token2push);
			cat_unref_ptr(token2push);
			
//			jag_virtual_parser_stack_push(vstack, jag_token_new_symbol_state(0, act-1));

//			if (debug) cat_log_debug("# Parse-ahead shifts Symbol #" + jag_parser_context_current_error_token(parserContext).sym + " into state #" + (act - 1));

			/* advance simulated input, if we run off the end, we are done */
			if (!jag_parser_base_advance_lookahead(parser_base, parserContext)) {
				break;
			}
		} else {
		/* < 0 encodes a reduce */
...
//				if (debug) cat_log_debug("# Parse-ahead accepts");
			...
			short lhs = jag_2d_array_get(parser_base->production_tab, (-act) - 1, 0);
			short rhs_size = jag_2d_array_get(parser_base->production_tab, (-act) - 1, 1);

			/* pop handle off the stack */
			int i;
			for(i = 0; i < rhs_size; i++) {
				jag_virtual_parser_stack_pop(vstack);
			}

//			if (debug) cat_log_debug("# Parse-ahead reduces: handle size = " + rhs_size + " lhs = #" + lhs + " from state #" + vstack.jag_parser_top());

			/* look up goto and push it onto the stack */

			int reduced_state = jag_parser_base_get_reduce(parser_base, jag_virtual_parser_stack_top(vstack), lhs);
			JagToken *token2push = scanner_iface->createToken(parserContext->scanner, 0, reduced_state, FALSE, FALSE, FALSE, 0, 0, 0, 0, NULL);
			jag_virtual_parser_stack_push(vstack, token2push);
			cat_unref_ptr(token2push);
//			jag_virtual_parser_stack_push(vstack, jag_token_new_symbol_state(0, reduced_state));
//			if (debug) cat_log_debug("# Goto state #" + vstack.jag_parser_top());
		}
	}
...
//		cat_log_debug("# Current Symbol is #" + jag_parser_context_current_error_token(parserContext).sym);
...
//	}
	...
	while (!parserContext->done_parsing) {
		...
		act = jag_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, error_token->sym);
		...
		if (act > 0) {
			...
			error_token->parse_state = act - 1;
			error_token->used_by_parser = TRUE;
//			if (debug) jag_parser_debug_shift(error_token);
			parser_context_class->shift(parserContext, error_token);
//			cat_unref_ptr(parserContext->current_token);
...
//			if (debug) cat_log_debug("# Current Symbol is #" + jag_parser_context_current_error_token(parserContext).sym);
		}
		/* if its less than zero, then it encodes a reduce action */
		else if (act < 0) {
			/* perform the action for the reduce */
...
//			if (debug) jag_parser_debug_reduce((-act) - 1, lhs_sym_num, handle_size);
...
//			jag_parser_base_report_fatal_error(parserContext, "Syntax error", lhs_sym);
			return;
		}
	}
