...
static void l_next_token(JagPILexer *lexer) {
	...
	JagPToken *cur_token = (JagPToken *) cat_array_wo_get(priv->tokens, priv->tokens_index);
	if (cur_token!=NULL && cur_token->kind==JAGP_KIND_EOF) {
		return;
	}
	int mx = 1000;
	priv->tokens_index++;
	while(priv->tokens_index>=cat_array_wo_size(priv->tokens)) {
		l_scan_real_token((JagPLexerImpl *) lexer);
		if (mx--<0) {
			break;
		}
	}
...
static JagPToken *l_token(JagPILexer *lexer) {
	JagPLexerImplPrivate *priv = jagp_lexer_impl_get_instance_private((JagPLexerImpl *) lexer);
	return (JagPToken *) cat_array_wo_get(priv->tokens, priv->tokens_index);
}


static JagPToken *l_token_ahead(JagPILexer *lexer, int lookahead) {
	JagPLexerImplPrivate *priv = jagp_lexer_impl_get_instance_private((JagPLexerImpl *) lexer);
	int offset = priv->tokens_index + 1 + lookahead;
	while(offset>=cat_array_wo_size(priv->tokens)) {
