...
static void jag_byt_op_array_length_init(JagBytOpArrayLength *instance) {
}
...
JagBytOpArrayLength *jag_byt_op_array_length_new(int offset) {
	JagBytOpArrayLength *result = g_object_new(JAG_BYT_TYPE_OP_ARRAY_LENGTH, NULL);
	cat_ref_anounce(result);
	jag_byt_abstract_mnemonic_construct((JagBytAbstractMnemonic *) result, OP_ARRAYLENGTH, offset, 1);
	return result;
}
