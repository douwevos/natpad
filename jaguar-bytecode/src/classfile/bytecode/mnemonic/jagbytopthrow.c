...
G_DEFINE_TYPE_WITH_CODE(JagBytOpThrow, jag_byt_op_throw, JAG_BYT_TYPE_ABSTRACT_MNEMONIC, // @suppress("Unused static function")
...
static void l_dispose(GObject *object) {
	...
	G_OBJECT_CLASS(jag_byt_op_throw_parent_class)->dispose(object);
...
static void l_finalize(GObject *object) {
	...
	G_OBJECT_CLASS(jag_byt_op_throw_parent_class)->finalize(object);
...
static CatStringWo *l_to_string(JagBytIMnemonic *self, JagBytLabelRepository *label_repository) {
	CatStringWo *result = cat_string_wo_new_with("athrow");
	return result;
}
