...
G_DEFINE_TYPE_WITH_CODE(JagAstInfixExpression, jag_ast_infix_expression, G_TYPE_OBJECT, {
		G_ADD_PRIVATE(JagAstInfixExpression)
...
static void l_dispose(GObject *object) {
	...
	JagAstInfixExpressionPrivate *priv = jag_ast_infix_expression_get_instance_private(instance);
	cat_unref_ptr(priv->e_expressions);
	cat_unref_ptr(priv->a_infix_operator_text);
	G_OBJECT_CLASS(jag_ast_infix_expression_parent_class)->dispose(object);
...
static void l_finalize(GObject *object) {
	...
	G_OBJECT_CLASS(jag_ast_infix_expression_parent_class)->finalize(object);
...
JagAstInfixExpression *jag_ast_infix_expression_new(JagBytMathOperator infix_operator, CatArrayWo *e_expressions) {
	...
	JagAstInfixExpressionPrivate *priv = jag_ast_infix_expression_get_instance_private(result);
...
static void l_expression_write(JagAstIExpression *self, JagAstWriter *out) {
	JagAstInfixExpression *instance = JAG_AST_INFIX_EXPRESSION(self);
	JagAstInfixExpressionPrivate *priv = jag_ast_infix_expression_get_instance_private(instance);
