...
#include "groedservice.h"
#include "editor/groededitorfactory.h"
#include "editor/groededitorconnector.h"

#include <logging/catlogdefs.h>
#define CAT_LOG_LEVEL CAT_LOG_WARN
#define CAT_LOG_CLAZZ "GroEdService"
#include <logging/catlog.h>

struct _GroEdServicePrivate {
...
G_DEFINE_TYPE_WITH_CODE(GroEdService, groed_service, G_TYPE_OBJECT,
		G_ADD_PRIVATE(GroEdService)
...
static void groed_service_class_init(GroEdServiceClass *clazz) {
	GObjectClass *object_class = G_OBJECT_CLASS(clazz);
	object_class->dispose = l_dispose;
	object_class->finalize = l_finalize;
}

static void groed_service_init(GroEdService *instance) {
}

static void l_dispose(GObject *object) {
	cat_log_detail("dispose:%p", object);
	GroEdService *instance = GROED_SERVICE(object);
	GroEdServicePrivate *priv = groed_service_get_instance_private(instance);
	cat_unref_ptr(priv->elk_service);
	cat_unref_ptr(priv->wor_service);
	cat_unref_ptr(priv->connector);
	G_OBJECT_CLASS(groed_service_parent_class)->dispose(object);
...
static void l_finalize(GObject *object) {
	...
	G_OBJECT_CLASS(groed_service_parent_class)->finalize(object);
	cat_log_detail("finalized:%p", object);
}


GroEdService *groed_service_new(WorService *wor_service, ElkService *elk_service) {
	GroEdService *result = g_object_new(GROED_TYPE_SERVICE, NULL);
	cat_ref_anounce(result);
	GroEdServicePrivate *priv = groed_service_get_instance_private(result);
	priv->wor_service = cat_ref_ptr(wor_service);
	priv->elk_service = cat_ref_ptr(elk_service);
	priv->connector = groed_editor_connector_new();
	return result;
}
...
CatS groed_s_ext_gro = CAT_S_DEF(".gro");


static void l_enlist_editor_factories(ElkIResourceHandler *self, CatArrayWo *e_enlist_to, MooNodeWo *node) {
	GroEdServicePrivate *priv = groed_service_get_instance_private(GROED_SERVICE(self));
