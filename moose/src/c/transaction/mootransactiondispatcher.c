...
G_DEFINE_TYPE (MooTransactionDispatcher, moo_transaction_dispatcher, G_TYPE_OBJECT)
...
static void moo_transaction_dispatcher_class_init(MooTransactionDispatcherClass *clazz) {
	...
	g_type_class_add_private(clazz, sizeof(MooTransactionDispatcherPrivate));
...
static void moo_transaction_dispatcher_init(MooTransactionDispatcher *instance) {
	MooTransactionDispatcherPrivate *priv = G_TYPE_INSTANCE_GET_PRIVATE(instance, MOO_TYPE_TRANSACTION_DISPATCHER, MooTransactionDispatcherPrivate);
	instance->priv = priv;
}
...
static void l_dispose(GObject *object) {
	...
	MooTransactionDispatcherPrivate *priv = instance->priv;
...
MooTransactionDispatcher *moo_transaction_dispatcher_new() {
	...
	MooTransactionDispatcherPrivate *priv = result->priv;
...
CatIIterator *moo_transaction_dispatcher_get_listeners_iter(MooTransactionDispatcher *dispatcher) {
	MooTransactionDispatcherPrivate *priv = MOO_TRANSACTION_DISPATCHER_GET_PRIVATE(dispatcher);
