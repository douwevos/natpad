...
static void vip_fs_file_class_init(VipFSFileClass *clazz) {
	...
	g_type_class_add_private(clazz, sizeof(VipFSFilePrivate));
...
static void vip_fs_file_init(VipFSFile *instance) {
	VipFSFilePrivate *priv = G_TYPE_INSTANCE_GET_PRIVATE(instance, VIP_TYPE_FS_FILE, VipFSFilePrivate);
	instance->priv = priv;
}
...
static void l_dispose(GObject *object) {
	...
	VipFSFilePrivate *priv = instance->priv;
...
VipFSFile *vip_fs_file_new(VipPath *path) {
	...
	VipFSFilePrivate *priv = result->priv;
...
VipPath *vip_fs_file_get_path(VipFSFile *fs_file) {
	return VIP_FS_FILE_GET_PRIVATE(fs_file)->path;
}
...
static CatStringWo *l_resource_get_name(VipIResource *self) {
	VipFSFilePrivate *priv = VIP_FS_FILE_GET_PRIVATE(self);
...
static gboolean l_resource_equal(VipIResource *res_a, VipIResource *res_b) {
	...
	VipFSFilePrivate *priv_a = VIP_FS_FILE_GET_PRIVATE(res_a);
	VipFSFilePrivate *priv_b = VIP_FS_FILE_GET_PRIVATE(res_b);
