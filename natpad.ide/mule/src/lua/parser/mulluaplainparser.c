...
#include "mulluaplainparser.h"
#define CAT_LOG_LEVEL CAT_LOG_WARN
#define CAT_LOG_CLAZZ "MulLuaPlainParser"
#include <logging/catlog.h>

struct _MulLuaPlainParserPrivate {
	MulLuaScanner *token_scanner;
...
G_DEFINE_TYPE_WITH_CODE(MulLuaPlainParser, mul_lua_plain_parser, G_TYPE_OBJECT,
		G_ADD_PRIVATE(MulLuaPlainParser)
...
static void mul_lua_plain_parser_class_init(MulLuaPlainParserClass *clazz) {
	GObjectClass *object_class = G_OBJECT_CLASS(clazz);
	object_class->dispose = l_dispose;
	object_class->finalize = l_finalize;
}

static void mul_lua_plain_parser_init(MulLuaPlainParser *instance) {
}

static void l_dispose(GObject *object) {
	cat_log_detail("dispose:%p", object);
	MulLuaPlainParser *instance = MUL_LUA_PLAIN_PARSER(object);
	MulLuaPlainParserPrivate *priv = mul_lua_plain_parser_get_instance_private(instance);
	...
	G_OBJECT_CLASS(mul_lua_plain_parser_parent_class)->dispose(object);
...
static void l_finalize(GObject *object) {
	...
	G_OBJECT_CLASS(mul_lua_plain_parser_parent_class)->finalize(object);
...
MulLuaPlainParser *mul_lua_plain_parser_new(MulLuaScanner *token_scanner, GroRunITokenFactory *token_factory, GroRunModel *model, DraKeywordPrinter *token_printer, DraKeywordPrinter *line_tag_printer) {
	MulLuaPlainParser *result = g_object_new(MUL_LUA_TYPE_PLAIN_PARSER, NULL);
	cat_ref_anounce(result);
	MulLuaPlainParserPrivate *priv = mul_lua_plain_parser_get_instance_private(result);
...
CatArrayWo *mul_lua_plain_parser_get_token_cash(MulLuaPlainParser *parser) {
	MulLuaPlainParserPrivate *priv = mul_lua_plain_parser_get_instance_private(parser);
	return priv->token_cash;
}


static int l_map_symbol(MulLuaPlainParserPrivate *priv, GroRunIToken *token);
...
void mul_lua_plain_parser_run(MulLuaPlainParser *parser) {
	...
	while(TRUE) {
		...
		if (token) {
			if (mul_lua_scanner_is_eof(priv->token_scanner, token)) {
				cat_unref_ptr(token);
				break;
			}
...
static int l_map_symbol(MulLuaPlainParserPrivate *priv, GroRunIToken *token) {
	if (GRORUN_IS_FULL_TOKEN(token)) {
		...
		switch(sym_index) {
			case MUL_LUA_SYM_KWNIL :
			case MUL_LUA_SYM_KWTRUE :
			case MUL_LUA_SYM_KWFALSE : return 12;
	//
			case MUL_LUA_SYM_KWIF :
			case MUL_LUA_SYM_KWELSE :
			case MUL_LUA_SYM_KWWHILE :
			case MUL_LUA_SYM_KWFOR :
			case MUL_LUA_SYM_KWBREAK :
			case MUL_LUA_SYM_KWCONCAT :
			case MUL_LUA_SYM_KWDO :
			case MUL_LUA_SYM_KWEND :
			case MUL_LUA_SYM_KWFUNCTION :
			case MUL_LUA_SYM_KWUNTIL :
			case MUL_LUA_SYM_KWREPEAT :
			case MUL_LUA_SYM_KWELSEIF : return 9;

			case MUL_LUA_SYM_LPAREN :
			case MUL_LUA_SYM_RPAREN :
			case MUL_LUA_SYM_LBRACE :
			case MUL_LUA_SYM_RBRACE : return 9;

			case MUL_LUA_SYM_LBRACK :
			case MUL_LUA_SYM_RBRACK : return 4;

			case MUL_LUA_SYM_STRING_CONTINUE :
			case MUL_LUA_SYM_STRING : return 5;

			case MUL_LUA_SYM_EOL_COMMENT : return 3;
...
static GroRunIToken *l_scanner_next(GroRunIScanner *self) {
	MulLuaPlainParser *plain_parser = MUL_LUA_PLAIN_PARSER(self);
	MulLuaPlainParserPrivate *priv = mul_lua_plain_parser_get_instance_private(plain_parser);
	...
	while(TRUE) {
		...
		if (token!=NULL) {
			...
			if ((user_index==MUL_LUA_SYM_EOL_COMMENT)) {
					cat_unref_ptr(token);
					continue;
			}
