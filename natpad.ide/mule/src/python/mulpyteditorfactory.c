...
#include "mulpyteditorfactory.h"

#include <logging/catlogdefs.h>
#define CAT_LOG_LEVEL CAT_LOG_WARN
#define CAT_LOG_CLAZZ "MulPytEditorFactory"
#include <logging/catlog.h>

struct _MulPytEditorFactoryPrivate {
	LeaIPanelOwner *panel_owner;
	MulPytEditorConnector *connector;
...
G_DEFINE_TYPE_WITH_CODE(MulPytEditorFactory, mul_pyt_editor_factory, G_TYPE_OBJECT,
		G_ADD_PRIVATE(MulPytEditorFactory)
...
static void mul_pyt_editor_factory_class_init(MulPytEditorFactoryClass *clazz) {
	GObjectClass *object_class = G_OBJECT_CLASS(clazz);
	object_class->dispose = l_dispose;
	object_class->finalize = l_finalize;
}

static void mul_pyt_editor_factory_init(MulPytEditorFactory *instance) {
}

static void l_dispose(GObject *object) {
	cat_log_detail("dispose:%p", object);
	MulPytEditorFactory *instance = MUL_PYT_EDITOR_FACTORY(object);
	MulPytEditorFactoryPrivate *priv = mul_pyt_editor_factory_get_instance_private(instance);
	cat_unref_ptr(priv->panel_owner);
	cat_unref_ptr(priv->connector);
	cat_unref_ptr(priv->document_io);
	cat_unref_ptr(priv->document_file);
	G_OBJECT_CLASS(mul_pyt_editor_factory_parent_class)->dispose(object);
...
static void l_finalize(GObject *object) {
	...
	G_OBJECT_CLASS(mul_pyt_editor_factory_parent_class)->finalize(object);
...
MulPytEditorFactory *mul_pyt_editor_factory_new(LeaIPanelOwner *panel_owner, MulPytEditorConnector *connector, ElkDocumentIO *document_io, VipIFile *document_file) {
	MulPytEditorFactory *result = g_object_new(MUL_TYPE_PYT_EDITOR_FACTORY, NULL);
	cat_ref_anounce(result);
	MulPytEditorFactoryPrivate *priv = mul_pyt_editor_factory_get_instance_private(result);
...
static void l_stringable_print(CatIStringable *self, struct _CatStringWo *append_to) {
	MulPytEditorFactory *instance = MUL_PYT_EDITOR_FACTORY(self);
	MulPytEditorFactoryPrivate *priv = mul_pyt_editor_factory_get_instance_private(instance);
...
static GtkWidget *l_create_editor(ElkIResourceEditorFactory *self, MooNodeWo *moo_node) {
	MulPytEditorFactoryPrivate *priv = mul_pyt_editor_factory_get_instance_private((MulPytEditorFactory *) self);
