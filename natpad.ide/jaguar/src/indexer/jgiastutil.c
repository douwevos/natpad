...
JgiTokenRange *jgi_ast_util_extract_name_root(CatArrayWo *e_token_array, int token_index) {
	...
	JagAstToken *cur_token = (JagAstToken *) cat_array_wo_get(e_token_array, ml_index);

	gboolean do_run_parenting = TRUE;
	int hold_index = -1;
	int dot_at_index = ml_index;

	cat_log_debug("token_array=%p, index=%d, cur_token=%o", e_token_array, token_index, cur_token);
	while(hold_index!=ml_index) {
		hold_index = ml_index;

		cur_token = (JagAstToken *) cat_array_wo_get(e_token_array, dot_at_index);
		switch(JAG_TOKEN(cur_token)->sym) {
			case JAG_SYMBOL_TERM_DOT : {
				ml_index = dot_at_index;
				if (ml_index==0) {
					break;
				}
				ml_index--;
				cur_token = (JagAstToken *) cat_array_wo_get(e_token_array, ml_index);
		...
		if (do_run_parenting) {
			JagAstToken *par = cur_token;
			while(do_run_parenting) {
				JagAstToken *next_par = par->parent_token;
