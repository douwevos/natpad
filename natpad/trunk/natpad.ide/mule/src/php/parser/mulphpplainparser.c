...
#include "mulphpplainparser.h"
#include <grossruntime.h>

#include <logging/catlogdefs.h>
#define CAT_LOG_LEVEL CAT_LOG_WARN
#define CAT_LOG_CLAZZ "MulPhpPlainParser"
#include <logging/catlog.h>

struct _MulPhpPlainParserPrivate {
	MulPhpScanner *token_scanner;
...
G_DEFINE_TYPE_WITH_CODE(MulPhpPlainParser, mul_php_plain_parser, G_TYPE_OBJECT,
		G_ADD_PRIVATE(MulPhpPlainParser)
...
static void mul_php_plain_parser_class_init(MulPhpPlainParserClass *clazz) {
	GObjectClass *object_class = G_OBJECT_CLASS(clazz);
	object_class->dispose = l_dispose;
	object_class->finalize = l_finalize;
}

static void mul_php_plain_parser_init(MulPhpPlainParser *instance) {
}

static void l_dispose(GObject *object) {
	cat_log_detail("dispose:%p", object);
	MulPhpPlainParser *instance = MUL_PHP_PLAIN_PARSER(object);
	MulPhpPlainParserPrivate *priv = mul_php_plain_parser_get_instance_private(instance);
	...
	G_OBJECT_CLASS(mul_php_plain_parser_parent_class)->dispose(object);
...
static void l_finalize(GObject *object) {
	...
	G_OBJECT_CLASS(mul_php_plain_parser_parent_class)->finalize(object);
...
MulPhpPlainParser *mul_php_plain_parser_new(MulPhpScanner *token_scanner, GroRunITokenFactory *token_factory, GroRunModel *model, DraKeywordPrinter *token_printer, DraKeywordPrinter *line_tag_printer) {
	MulPhpPlainParser *result = g_object_new(MUL_PHP_TYPE_PLAIN_PARSER, NULL);
	cat_ref_anounce(result);
	MulPhpPlainParserPrivate *priv = mul_php_plain_parser_get_instance_private(result);
...
static int l_map_symbol(MulPhpPlainParserPrivate *priv, GroRunIToken *token);


void mul_php_plain_parser_run(MulPhpPlainParser *parser) {
	MulPhpPlainParserPrivate *priv = mul_php_plain_parser_get_instance_private(parser);
	...
	while(TRUE) {
		...
		if (token) {
			if (mul_php_scanner_is_eof(priv->token_scanner, token)) {
				cat_unref_ptr(token);
				break;
			}
...
static int l_map_symbol(MulPhpPlainParserPrivate *priv, GroRunIToken *token) {
	if (GRORUN_IS_FULL_TOKEN(token)) {
		...
		switch(sym_index) {
			...
			case MUL_PHP_SYM_T_PLUS_EQUAL :
			case MUL_PHP_SYM_T_MINUS_EQUAL :
			case MUL_PHP_SYM_OP_ASSIGN :
				return 9;

			case MUL_PHP_SYM_COLON :
				return 7;

			case MUL_PHP_SYM_T_STRING :
				return 8;

			case MUL_PHP_SYM_COMMENT :
				return 12;
...
static GroRunIToken *l_scanner_next(GroRunIScanner *self) {
	MulPhpPlainParser *plain_parser = MUL_PHP_PLAIN_PARSER(self);
	MulPhpPlainParserPrivate *priv = mul_php_plain_parser_get_instance_private(plain_parser);
	...
	while(TRUE) {
		...
		if (token!=NULL) {
			...
			if ((user_index==MUL_PHP_SYM_COMMENT)) {
					cat_unref_ptr(token);
					continue;
			}
		}
