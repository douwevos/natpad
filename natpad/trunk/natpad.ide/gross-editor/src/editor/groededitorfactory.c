...
#include "groededitorfactory.h"
#include "groededitorpanel.h"

#include <logging/catlogdefs.h>
#define CAT_LOG_LEVEL CAT_LOG_WARN
#define CAT_LOG_CLAZZ "GroEdEditorFactory"
#include <logging/catlog.h>

struct _GroEdEditorFactoryPrivate {
...
G_DEFINE_TYPE_WITH_CODE(GroEdEditorFactory, groed_editor_factory, G_TYPE_OBJECT,
		G_ADD_PRIVATE(GroEdEditorFactory)
...
static void groed_editor_factory_class_init(GroEdEditorFactoryClass *clazz) {
	GObjectClass *object_class = G_OBJECT_CLASS(clazz);
	object_class->dispose = l_dispose;
	object_class->finalize = l_finalize;
}

static void groed_editor_factory_init(GroEdEditorFactory *instance) {
}

static void l_dispose(GObject *object) {
	cat_log_detail("dispose:%p", object);
	GroEdEditorFactory *instance = GROED_EDITOR_FACTORY(object);
	GroEdEditorFactoryPrivate *priv = groed_editor_factory_get_instance_private(instance);
	cat_unref_ptr(priv->document_file);
	cat_unref_ptr(priv->document_io);
	cat_unref_ptr(priv->connector);
	cat_unref_ptr(priv->panel_owner);
	G_OBJECT_CLASS(groed_editor_factory_parent_class)->dispose(object);
...
static void l_finalize(GObject *object) {
	...
	G_OBJECT_CLASS(groed_editor_factory_parent_class)->finalize(object);
...
GroEdEditorFactory *groed_editor_factory_new(LeaIPanelOwner *panel_owner, GroEdEditorConnector *connector, ElkDocumentIO *document_io, VipIFile *document_file) {
	GroEdEditorFactory *result = g_object_new(GROED_TYPE_EDITOR_FACTORY, NULL);
	cat_ref_anounce(result);
	GroEdEditorFactoryPrivate *priv = groed_editor_factory_get_instance_private(result);
...
static GtkWidget *l_create_editor(ElkIResourceEditorFactory *self, MooNodeWo *moo_node) {
	GroEdEditorFactoryPrivate *priv = groed_editor_factory_get_instance_private((GroEdEditorFactory *) self);
	...
	GroEdEditorPanel *result = groed_editor_panel_new(priv->panel_owner, document_bin, priv->connector);
