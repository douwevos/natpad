...
#include "jageditorfactory.h"
#include "jageditorpanel.h"
#include <elk.h>

#include <logging/catlogdefs.h>
#define CAT_LOG_LEVEL CAT_LOG_WARN
#define CAT_LOG_CLAZZ "JagEditorFactory"
#include <logging/catlog.h>

struct _JagEditorFactoryPrivate {
...
G_DEFINE_TYPE_WITH_CODE(JagEditorFactory, jag_editor_factory, G_TYPE_OBJECT, {
		G_ADD_PRIVATE(JagEditorFactory)
...
static void jag_editor_factory_class_init(JagEditorFactoryClass *clazz) {
...
static void jag_editor_factory_init(JagEditorFactory *instance) {
}

static void l_dispose(GObject *object) {
	cat_log_detail("dispose:%p", object);
	JagEditorFactory *instance = JAG_EDITOR_FACTORY(object);
	JagEditorFactoryPrivate *priv = jag_editor_factory_get_instance_private(instance);
	...
	G_OBJECT_CLASS(jag_editor_factory_parent_class)->dispose(object);
...
static void l_finalize(GObject *object) {
	...
	G_OBJECT_CLASS(jag_editor_factory_parent_class)->finalize(object);
...
JagEditorFactory *jag_editor_factory_new(LeaIPanelOwner *panel_owner, JagEditorConnector *connector, ElkDocumentIO *document_io, VipIFile *document_file, JagIndexer *indexer) {
	JagEditorFactory *result = g_object_new(JAG_TYPE_EDITOR_FACTORY, NULL);
	cat_ref_anounce(result);
	JagEditorFactoryPrivate *priv = jag_editor_factory_get_instance_private(result);
...
static GtkWidget *l_create_editor(ElkIResourceEditorFactory *self, MooNodeWo *moo_node) {
	JagEditorFactoryPrivate *priv = jag_editor_factory_get_instance_private((JagEditorFactory *) self);
	...
	JagEditorPanel *result = jag_editor_panel_new(priv->panel_owner, document_bin, priv->connector, priv->indexer);
...
static void l_stringable_print(CatIStringable *self, struct _CatStringWo *append_to) {
	JagEditorFactory *instance = JAG_EDITOR_FACTORY(self);
	JagEditorFactoryPrivate *priv = jag_editor_factory_get_instance_private(instance);
