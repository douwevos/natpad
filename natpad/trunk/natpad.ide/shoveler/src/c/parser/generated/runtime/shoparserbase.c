#include "shoparserbase.h"

#include <logging/catlogdefs.h>
#define CAT_LOG_LEVEL CAT_LOG_WARN
#define CAT_LOG_CLAZZ "ShoParserBase"
#include <logging/catlog.h>

G_DEFINE_TYPE(ShoParserBase, sho_parser_base, G_TYPE_OBJECT)

static void _dispose(GObject *object);

static void sho_parser_base_class_init(ShoParserBaseClass *clazz) {
	GObjectClass *object_class = G_OBJECT_CLASS(clazz);
	object_class->dispose = _dispose;
}

static void sho_parser_base_init(ShoParserBase *obj) {
}

static void _dispose(GObject *object) {
//	ShoParserBase *instance = SHO_PARSER_BASE(object);
	// TODO
}


ShoParserBase *sho_parser_base_new() {
	ShoParserBase *result = g_object_new(SHO_TYPE_PARSER_BASE, NULL);
...
ShoToken *sho_parser_base_parse(ShoParserBase *parser_base, ShoParserContext *parserContext) {
	...
	ShoToken *lhs_sym = NULL;

	/* information about production being reduced with */
	short handle_size, lhs_sym_num;


	const ShoParserBaseClass *base_class = SHO_PARSER_BASE_GET_CLASS(parser_base);
	const ShoParserContextClass *parser_context_class = SHO_PARSER_CONTEXT_GET_CLASS(parserContext);
	...
	ShoIScannerInterface *scanner_iface = SHO_ISCANNER_GET_INTERFACE(parserContext->scanner);
	
	ShoToken *start_token = scanner_iface->createToken(parserContext->scanner, 0, base_class->start_state(parser_base), FALSE, FALSE, FALSE, 0, 0, 0, 0, NULL);
	parser_context_class->push(parserContext, start_token);	
	cat_unref_ptr(start_token);
//	parser_context_class->push(parserContext, sho_token_new_symbol_state(0, base_class->start_state(parser_base)));
	parserContext->tos = 0;
...
//			throw new Error("Symbol recycling detected (fix your scanner).");
		...
		sho_parser_context_debug(parserContext);
		sho_token_dump(parser_context_class->peek(parserContext));

		if (parserContext->current_token!=NULL) {
			sho_token_dump(parserContext->current_token);
		}
		...
		act = sho_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, parserContext->current_token->sym);
		...
		if (act > 0) {
			...
			cat_log_debug("lhs_sym %d, %s", lhs_sym->sym, lhs_sym->symbol_text);


			/* look up information about the production */
			lhs_sym_num = sho_2d_array_get(parser_base->production_tab, (-act) - 1, 0);
			handle_size = sho_2d_array_get(parser_base->production_tab, (-act) - 1, 1);

			act = sho_parser_base_get_reduce(parser_base, parser_context_class->peek_at(parserContext, handle_size)->parse_state, lhs_sym_num);
...
//			act = sho_parser_base_get_reduce(parser_base, parser_context_class->peek(parserContext)->parse_state, lhs_sym_num);
//
//			/* shift to that state */
//			lhs_sym->parse_state = act;
//			lhs_sym->used_by_parser = TRUE;
...
//			sho_parser_report_syntax_error(parserContext->current_token);

			/* try to error recover */
			if (!sho_parser_base_error_recovery(parser_base, parserContext, FALSE)) {
				/* if that fails give up with a fatal syntax error */
//				sho_parser_unrecovered_syntax_error(parser_base, parserContext, parserContext->current_token);

				/* just in case that wasn't fatal enough, end parse */
				sho_parser_context_done_parsing(parserContext);
				lhs_sym = NULL;
			} else {
...
short sho_parser_base_get_action(ShoParserBase *parser_base, int state, int sym) {
	...
	int row_length = 0;
	short *row = sho_2d_array_get_row(parser_base->action_tab, state, &row_length);
...
gboolean sho_parser_base_error_recovery(ShoParserBase *parser_base, ShoParserContext *parserContext, gboolean debug) {
	ShoParserBaseClass *base_class = SHO_PARSER_BASE_GET_CLASS(parser_base);
	 ...
	 */
	if (!sho_parser_base_find_recovery_config(parser_base, parserContext, debug)) {
//		if (debug) cat_log_debug("# Error recovery fails");
		return FALSE;
	}

	/* read ahead to create lookahead we can parse multiple times */
	sho_parser_base_read_lookahead(parser_base, parserContext);
	...
	while(TRUE) {
		...
		if (sho_parser_base_try_parse_ahead(parser_base, parserContext, debug)) {
			break;
		}

		/* if we are now at EOF, we have failed */
		ShoToken *bottomToken = (ShoToken *) cat_array_wo_get(parserContext->e_lookahead, 0);
		if (bottomToken==NULL) {
...
//		cat_log_debug("# Consuming Symbol #" + parserContext->lookahead[0].sym);
		...
		sho_parser_base_restart_lookahead(parser_base, parserContext);
	}
	...
	sho_parser_base_parse_lookahead(parser_base, parserContext, debug);
...
short sho_parser_base_get_reduce(ShoParserBase *parser_base, int state, int sym) {
	...
	short *row = sho_2d_array_get_row(parser_base->reduce_tab, state, &row_length);
...
gboolean sho_parser_base_shift_under_error(ShoParserBase *parser_base, ShoParserContext *parserContext) {
	...
	int errorSym = SHO_PARSER_BASE_GET_CLASS(parser_base)->error_symbol(parser_base);
	ShoParserContextClass *parser_context_class = SHO_PARSER_CONTEXT_GET_CLASS(parserContext);
	return sho_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, errorSym) > 0;
}
...
gboolean sho_parser_base_find_recovery_config(ShoParserBase *parser_base, ShoParserContext *parserContext, gboolean debug) {
	...
	while (!sho_parser_base_shift_under_error(parser_base, parserContext)) {
		/* pop the stack */
//		if (debug) cat_log_debug("# Pop stack by one, state was # " + parser_context_class->peek(parserContext).parse_state);
	...
	act = sho_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, base_class->error_symbol(parser_base));
//	if (debug) {
...
//	error_token = sho_token_new_full(base_class->error_symbol(parser_base), FALSE, left_pos, left_row, right_pos, right_row, NULL);
//	error_token->symbol_text= "error"; 
...
//		parserContext.lookahead = new ShoToken[sho_parser_error_sync_size()];
	} else {
...
void sho_parser_base_restart_lookahead(ShoParserBase *parser_base, ShoParserContext *parserContext) {
	...
	int eofSymbol = SHO_PARSER_BASE_GET_CLASS(parser_base)->eof_symbol(parser_base);
	parser_context_class->scanNext(parserContext, eofSymbol);
...
gboolean sho_parser_base_try_parse_ahead(ShoParserBase *parser_base, ShoParserContext *parserContext, gboolean debug) {
	...
	ShoVirtualParseStack *vstack = sho_virtual_parse_stack_new(parserContext->e_stack);
	...
	while(TRUE) {
		ShoToken *error_token = sho_parser_context_current_error_token(parserContext);
		...
		int act = sho_parser_base_get_action(parser_base, sho_virtual_parser_stack_top(vstack), error_token->sym);
		...
		if (act > 0) {
			...
			ShoToken *token2push = scanner_iface->createToken(parserContext->scanner, 0, act-1, FALSE, FALSE, FALSE, 0, 0, 0, 0, NULL);
			sho_virtual_parser_stack_push(vstack, token2push);
			cat_unref_ptr(token2push);
			
//			sho_virtual_parser_stack_push(vstack, sho_token_new_symbol_state(0, act-1));

//			if (debug) cat_log_debug("# Parse-ahead shifts Symbol #" + sho_parser_context_current_error_token(parserContext).sym + " into state #" + (act - 1));

			/* advance simulated input, if we run off the end, we are done */
			if (!sho_parser_base_advance_lookahead(parser_base, parserContext)) {
				break;
			}
		} else {
		/* < 0 encodes a reduce */
...
//				if (debug) cat_log_debug("# Parse-ahead accepts");
			...
			short lhs = sho_2d_array_get(parser_base->production_tab, (-act) - 1, 0);
			short rhs_size = sho_2d_array_get(parser_base->production_tab, (-act) - 1, 1);

			/* pop handle off the stack */
			int i;
			for(i = 0; i < rhs_size; i++) {
				sho_virtual_parser_stack_pop(vstack);
			}

//			if (debug) cat_log_debug("# Parse-ahead reduces: handle size = " + rhs_size + " lhs = #" + lhs + " from state #" + vstack.sho_parser_top());

			/* look up goto and push it onto the stack */

			int reduced_state = sho_parser_base_get_reduce(parser_base, sho_virtual_parser_stack_top(vstack), lhs);
			ShoToken *token2push = scanner_iface->createToken(parserContext->scanner, 0, reduced_state, FALSE, FALSE, FALSE, 0, 0, 0, 0, NULL);
			sho_virtual_parser_stack_push(vstack, token2push);
			cat_unref_ptr(token2push);
//			sho_virtual_parser_stack_push(vstack, sho_token_new_symbol_state(0, reduced_state));
//			if (debug) cat_log_debug("# Goto state #" + vstack.sho_parser_top());
		}
	}
...
//		cat_log_debug("# Current Symbol is #" + sho_parser_context_current_error_token(parserContext).sym);
...
//	}
	...
	while (!parserContext->done_parsing) {
		...
		act = sho_parser_base_get_action(parser_base, parser_context_class->peek(parserContext)->parse_state, error_token->sym);
		...
		if (act > 0) {
			...
			error_token->parse_state = act - 1;
			error_token->used_by_parser = TRUE;
//			if (debug) sho_parser_debug_shift(error_token);
			parser_context_class->shift(parserContext, error_token);
//			cat_unref_ptr(parserContext->current_token);
...
//			if (debug) cat_log_debug("# Current Symbol is #" + sho_parser_context_current_error_token(parserContext).sym);
		}
		/* if its less than zero, then it encodes a reduce action */
		else if (act < 0) {
			/* perform the action for the reduce */
...
//			if (debug) sho_parser_debug_reduce((-act) - 1, lhs_sym_num, handle_size);
...
//			sho_parser_base_report_fatal_error(parserContext, "Syntax error", lhs_sym);
			return;
		}
	}
