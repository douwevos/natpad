...
#include "mulluaeditorfactory.h"

#include <logging/catlogdefs.h>
#define CAT_LOG_LEVEL CAT_LOG_WARN
#define CAT_LOG_CLAZZ "MulLuaEditorFactory"
#include <logging/catlog.h>

struct _MulLuaEditorFactoryPrivate {
	LeaIPanelOwner *panel_owner;
	MulLuaEditorConnector *connector;
...
G_DEFINE_TYPE_WITH_CODE(MulLuaEditorFactory, mul_lua_editor_factory, G_TYPE_OBJECT,
		G_ADD_PRIVATE(MulLuaEditorFactory)
...
static void mul_lua_editor_factory_class_init(MulLuaEditorFactoryClass *clazz) {
	GObjectClass *object_class = G_OBJECT_CLASS(clazz);
	object_class->dispose = l_dispose;
	object_class->finalize = l_finalize;
}

static void mul_lua_editor_factory_init(MulLuaEditorFactory *instance) {
}

static void l_dispose(GObject *object) {
	cat_log_detail("dispose:%p", object);
	MulLuaEditorFactory *instance = MUL_LAU_EDITOR_FACTORY(object);
	MulLuaEditorFactoryPrivate *priv = mul_lua_editor_factory_get_instance_private(instance);
	cat_unref_ptr(priv->panel_owner);
	cat_unref_ptr(priv->connector);
	cat_unref_ptr(priv->document_io);
	cat_unref_ptr(priv->document_file);
	G_OBJECT_CLASS(mul_lua_editor_factory_parent_class)->dispose(object);
...
static void l_finalize(GObject *object) {
	...
	G_OBJECT_CLASS(mul_lua_editor_factory_parent_class)->finalize(object);
...
MulLuaEditorFactory *mul_lua_editor_factory_new(LeaIPanelOwner *panel_owner, MulLuaEditorConnector *connector, ElkDocumentIO *document_io, VipIFile *document_file) {
	MulLuaEditorFactory *result = g_object_new(MUL_TYPE_LAU_EDITOR_FACTORY, NULL);
	cat_ref_anounce(result);
	MulLuaEditorFactoryPrivate *priv = mul_lua_editor_factory_get_instance_private(result);
...
static void l_stringable_print(CatIStringable *self, struct _CatStringWo *append_to) {
	MulLuaEditorFactory *instance = MUL_LAU_EDITOR_FACTORY(self);
	MulLuaEditorFactoryPrivate *priv = mul_lua_editor_factory_get_instance_private(instance);
...
static GtkWidget *l_create_editor(ElkIResourceEditorFactory *self, MooNodeWo *moo_node) {
	MulLuaEditorFactoryPrivate *priv = mul_lua_editor_factory_get_instance_private((MulLuaEditorFactory *) self);
