...
//		if (debug) cat_log_debug("# Error recovery fails");
	...
	while(TRUE) {
		...
		SmplToken *bottomToken = (SmplToken *) cat_array_wo_get(parserContext->e_lookahead, 0);
		if (bottomToken==NULL) {
...
gboolean smpl_parser_base_try_parse_ahead(SmplParserBase *parser_base, SmplParserContext *parserContext, gboolean debug) {
	...
	SmplVirtualParseStack *vstack = smpl_virtual_parse_stack_new(parserContext->e_stack);
