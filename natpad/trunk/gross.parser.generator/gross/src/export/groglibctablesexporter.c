...
//
//void gro_glibc_export_tables(GroModel *model, GroStateMachine *state_machine, GFile *output_path, GroClassName *cname) {
...
//		cat_string_wo_format(buf, "\n");
//		cat_string_wo_format(buf, "GroParserConfig %O_%O_definitions;\n", cname->prefix_lower, cname->name_lower);
//		cat_string_wo_format(buf, "\n");
...
//
//		cat_string_wo_format(buf, "\nGroParserConfig %O_%O_definitions = {\n", cname->prefix_lower, cname->name_lower);
...
//	cat_string_wo_format(out, "const int %O_%O_production_table[] = {\n", cname->prefix_lower, cname->name_lower);
...
//	cat_string_wo_format(out, "const int %O_%O_action_table[] = {\n", cname->prefix_lower, cname->name_lower);
...
//				cat_log_error("eeeeeeeeee:entry_idx=%d, act=%O, fork_index=%d", entry_idx, act_sub, fork_index);
...
//						cat_log_error("eeeeeeeeee:act=%d", fork_table_row[entry_idx-1]);
//					} break;
//					case GRO_ACTION_KIND_SHIFT : {
...
//		cat_string_wo_format(out, "const int %O_%O_fork_table[] = {\n", cname->prefix_lower, cname->name_lower);
//		l_emit_table(out, fork_row_size, fork_table, max_fork_action_count, fork_action_row_cnt, 0);
//		cat_string_wo_append_chars(out, "};\n\n");
//		result = TRUE;
//	}
//	return result;
...
//	cat_string_wo_format(out, "const int %O_%O_reduce_table[] = {\n", cname->prefix_lower, cname->name_lower);
...
//#define TAB_PREFIX "\t"
//
//	// print a string array encoding the given int[][] array.
